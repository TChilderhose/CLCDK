...
local function UnholyMove(frame)
	...
	-- Raise Abomination
	if (numRunes >= 1 and CLCDK.IsSpellNameOffCD("Raise Abomination")) then
	...
	local festWounds = CLCDK.FindTargetDebuff("Festering Wound")
	...
	if (numRunes >= 1 and CLCDK.CanShowExecute("target") and CLCDK.IsSpellNameOffCD("Soul Reaper")) then
	...
	if (numRunes >= 1 and CLCDK.GetSpecDiseaseRemaining() <= 3 and CLCDK.GetSpellNameCDRemaining("Dark Transformation") > 12) then
	...
	if (numRunes >= 2 and numFestWounds <= 3) then
...
local function FrostMove(frame)
	...
	if (not aoeSet and frame.AOE and numRunes >= 1 and not CLCDK.GetPlayerBuff("Death and Decay") and CLCDK.IsSpellNameOffCD("Death and Decay")) then
